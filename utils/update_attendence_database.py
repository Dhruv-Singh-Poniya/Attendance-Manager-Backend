...
from utils.data_models import attendance_record
    ...
    for data in attendence:
        ...
        try:
            ...
            if str(e.pgcode) != str(UNIQUE_VIOLATION):
                return {'status': 'error', 'message': f'{username}: SQL Unique Violation Error'}
            conn.rollback()
        ...
        try:
            ...
            if len(latest_attendence) !=2:
                return {'status': 'error', 'message': f'{username}: Not Enough Data for Notifications'}
            current_attendence, previous_attendence = attendance_record(*latest_attendence[0]), attendance_record(*latest_attendence[1])
            
            if current_attendence.max_hours == previous_attendence.max_hours:
                continue
            
            # Logic for Notifications
            max_hours_diff = current_attendence.max_hours - previous_attendence.max_hours
            present_hours_diff = current_attendence.attended_hours - previous_attendence.attended_hours
            absent_hours_diff = current_attendence.absent_hours - previous_attendence.absent_hours
            
            if present_hours_diff + absent_hours_diff != max_hours_diff:
                return {'status': 'error', 'message': f'{username}: Invalid Attendance Data'}
            ...
            if absent_hours_diff > 0:
                notifications.push_notification(
                    ...
                    previous_date=str(previous_attendence.date.strftime('%d-%m-%Y')),
                    previous_time=str(previous_attendence.time.strftime('%H:%M:%S')),
            ...
            return {'status': 'error', 'message': f'{username}: Error in Notifications, {e}'}
